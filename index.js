...
async function doublebet(_game, _user, _amount) {
    ...
    while (!double_bet_success) {
        try {
            ...
            const tx_double_bet = await bj_game_factory.doubleBet(_game, _user, ethers.utils.parseEther(_amount), {gasLimit: 2000000, gasPrice:"16000000000"})
...
async function declare_winner(_user, _game, _isUserWin, _isDraw) {
    ...
    while (!declare_success) {
        try {
            ...
            const declare_tx = await bj_game_factory.declareWinner(_game, _isUserWin, _isDraw, {gasLimit: 2000000, gasPrice:"16000000000"})
    ...
    while (!reward_success) {
        try {
            ...
            if(_isUserWin == true){
                ...
                const collect_tx = await bj_game.collect( {gasLimit: 2000000, gasPrice:"16000000000"})
                ...
                const bj_game = new ethers.Contract(_game, bj_game_abi, admin_signer)
                const recover_tx = await bj_game.recover( {gasLimit: 2000000, gasPrice:"16000000000"})
